...
- 一个由C/C++编译的程序占用的内存有以下几个部分：
  ...
  >   程序结束后由系统释放。
  >
  > - 文字常量区 —常量字符串就是放在这里的。常量区是全局区中划分的一个小区域，里面存放的是常量，如const修饰的全局变量、字符串常量等
  > - 程序代码区—存放函数体的二进制代码。
...
- \_\_VA_ARGS\_\_为可变参数宏，实现思想就是宏定义中参数列表的最后一个参数为省略号（也就是三个点）。这样预定义宏\_ _VA\_ARGS\_\_就可以被用在替换部分中，替换省略号所代表的字符串。==注意当可变参数量可能为0时，加##用来支持0个可变参数的情况。==比如下：
...
[C++编译链接](C:\Users\CARRYCHOU\Desktop\编译相关)
