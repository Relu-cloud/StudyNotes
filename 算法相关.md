...
- Dijkstra算法的时间复杂度为`O((V+E)logV)`，其中V表示图中的节点数，E表示图中的边数。具体来说，算法的时间复杂度主要取决于两个部分：节点的遍历和优先队列的操作。|==有疑问，如果按BFS+DP表理解，应该是O(ElogE)，在存在父权的情况下，每条边只会被访问一次。==
  ...
  在优先队列的操作中，Dijkstra算法需要将节点按照距离值进行排序，并选择距离值最小的节点进行处理。在每次选择节点后，需要更新与该节点相邻的节点的距离值。这个过程中，每个节点最多会被处理一次，而每次处理时需要更新其相邻节点的距离值。由于优先队列的插入和删除操作的时间复杂度为O(logV)，因此总的时间复杂度为O((V+E)logV)。【说是logV还是logE都可以，即使E=V2，也就是多了一倍】
